...
from sqlalchemy import text, desc, func, case, extract
...
from src.persistence.exceptions import InspectionPersistenceError, InspectionNotFoundError
from src.persistence.schemas import InspectionHistoryItem
from src.api.schemas import InspectionResponse, DefectSchema, BBox, PLCDispatchInfo, InspectionSummary
...
class SQLAlchemyInspectionRepository(InspectionRepository):
    ...
    def get_by_id(self, inspection_id: str) -> Optional[InspectionHistoryItem]:
        with self.session_factory() as session:
            record = session.query(InspectionRecord).filter(InspectionRecord.inspection_id == inspection_id).first()
            if not record:
                return None
        ...
        with self.session_factory() as session:
            ...
            if filters:
                if "decision" in filters and filters["decision"]:
                    query = query.filter(InspectionRecord.decision == filters["decision"])
                if "severity" in filters and filters["severity"]:
                    query = query.filter(InspectionRecord.severity == filters["severity"])
                ...
                if "defect_class" in filters and filters["defect_class"]:
                    query = query.join(DefectModel).filter(DefectModel.class_name == filters["defect_class"])
    ...
    def _map_to_history_item(self, record: InspectionRecord) -> InspectionHistoryItem:
        ...
        if record.affected_classes:
            affected_classes = [c.strip() for c in record.affected_classes.split(",") if c.strip()]
    ...
    def get_inspection_stats(self, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> Dict[str, Any]:
        with self.session_factory() as session:
            query = session.query(
                ...
                func.sum(case((InspectionRecord.decision == "PASS", 1), else_=0)).label("pass_count"),
                func.sum(case((InspectionRecord.decision == "REVIEW", 1), else_=0)).label("review_count"),
                func.sum(case((InspectionRecord.decision == "REJECT", 1), else_=0)).label("reject_count"),
    ...
    def get_defect_distribution(self, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> List[Dict[str, Any]]:
        with self.session_factory() as session:
            ...
            query = query.group_by(DefectModel.class_name).order_by(desc("total_occurrences"))
            
            return [{"class_name": row.class_name, "total_occurrences": row.total_occurrences} for row in query.all()]
        ...
        with self.session_factory() as session:
            ...
            for row in query.all():
                if row.bucket:
                    ...
                    dt = datetime.strptime(row.bucket, time_fmt).replace(tzinfo=timezone.utc)
                    results.append({
                        "timestamp": dt,
